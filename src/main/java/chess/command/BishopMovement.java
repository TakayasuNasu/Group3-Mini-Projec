...
public class BishopMovement implements Movement {
  ...
  /**
   * Search if the piece can move in Diagonal way.
   * There are 4 ways to search and each ways are selected by {@code direction}
   *  {  ↗:-9 ,  ↖:-11,  ↘:11,  ↙:9 }
   *
   *
  ...
  public ArrayList<Integer> searchDiagonal(int cur, ArrayList<Integer> positions , int direction){
    for (int i = 1; i < 8; i++) {
      ...
      if (!this.valid.canMove(cur + direction * i)) return positions;
